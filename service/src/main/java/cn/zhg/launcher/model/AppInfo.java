...
import cn.zhg.launcher.parser.ManifestXmlParser;
...
{ 
	...
	public void updateManifestXml()
	{
		...
		try
		{
			...
			parser.parseApk(sourceDir);
			this.manifestXml= parser.getXML();
		} catch (Throwable e)
		{
			Logger.d(packageName + "读取AndroidManifest详细内容错误:" + e.getMessage());
		}
	}
	/**
	 * 进行数据更新
...
//		manifestXml=tryGetManifestXml();
	...
	private void updateBaseData(final PackageInfo pp)
	{
		...
		{
			this.minSdkVersion=app.minSdkVersion;
		}
		...
		if (sourceDir != null)
		{
			...
			fileSizeStr = SomeUtil.getFileSize(fileSize);
		}
